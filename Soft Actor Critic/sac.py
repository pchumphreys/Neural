...
class Policy_Discrete(MLP):
	...
	# For now assume that discrete action space, such that tractable - obviously this slightly defeats the point
	def init(self,s_size,a_size,n_hidden,n_layers):
...
class SAC():
	...
	def init_Q_net_training(self):
		...
		Q_Loss = 0.5*tf.reduce_sum(tf.square(Qs - self.rewards_placeholder - self.discount * target_Vs))
