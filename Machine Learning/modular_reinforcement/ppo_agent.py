...
class PPO_agent(Base_Agent):
	...
	def __init__(self,n_inputs,n_outputs,**params):
		...
		super(PPO_agent,self).__init__(**params)
		...
		self.clip_param = params['agent_params'].pop('clip_param',0.1)
		...
		self.target_policy = Policy_Discrete(self.obs,self.n_outputs,params['network_spec'],scope='target_policy',**params['policy_params'])
		self.target_policy_params = self.target_policy.get_params_internal()
	...
	def _init_training_op(self):
		...
		with tf.variable_scope('policy_loss'):
			...
			ratio_p_to_pt = tf.exp(tf.reduce_sum((self.policy.log_policy_outputs - self.target_policy.log_policy_outputs)* self.actions,axis=1)) 
			surr1 = ratio_p_to_pt * self.advs 
			surr2 = tf.clip_by_value(ratio_p_to_pt, 1.0 - self.clip_param, 1.0 + self.clip_param) *self.advs 
			self.raw_policy_loss = -tf.reduce_mean(tf.minimum(surr1, surr2)) 
